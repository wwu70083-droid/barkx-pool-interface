...
若满足如下条件之一，则购买按钮渲染为跳过，按优先度显示其中一个原因：
...
如果铸造需求大于等于 0.000000000001 LP，就渲染添加流动性按钮，并显示将提交的 BARKX 和 USDT 数量。

如果铸造需求小于 0.000000000001 LP（包括负值），就渲染跳过按钮。
...
如果可存量大于等于 0.000000000001 LP，就渲染存入 LP 按钮，并显示将提交的 LP 数量。

如果可存量小于 0.000000000001 LP，就渲染关闭按钮。
